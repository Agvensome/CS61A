...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # evaluate expression bound
        rest = expressions.rest
        eval_exp = scheme_eval(rest.first, env)
        # bind symbol to value in env frame
        env.define(signature, eval_exp)
        return signature
        # END PROBLEM 4
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        function_name, formal_pair, body_pair = signature.first, signature.rest, expressions.rest
        procedure = do_lambda_form(Pair(formal_pair, body_pair), env)
        env.define(function_name, procedure)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        names, vals = Pair(symbol, names), Pair(scheme_eval(expr, env), vals)
