...
class Ant(Insect):
    ...
    is_container = False
    # ADD CLASS ATTRIBUTES HERE
    is_double = True                # Problem 12
    blocks_path = True              # Problem EC 3
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            # place.ant is ContainerAnt()
            if place.ant.can_contain(self):
                place.ant.store_ant(self)
            # self is ContainerAnt()
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # Only solute Problem 3
        # cur_place = self.place
        # while not cur_place.is_hive and cur_place.entrance:
        #     if cur_place.bees:
        #         return random_bee(cur_place.bees)
        #     cur_place = cur_place.entrance
        # return None

        cur_place, pos = self.place, self.lower_bound
        for _ in range(pos): 
            if cur_place.entrance:
                cur_place = cur_place.entrance
            else:
                return None
                
        while not cur_place.is_hive and cur_place.entrance and pos <= self.upper_bound:
            if cur_place.bees:
                return random_bee(cur_place.bees)
            cur_place = cur_place.entrance
            pos += 1
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt does select a random Bee from its place and deal damage to the Bee equal to the Bee's health, eating it whole. 
    After eating a Bee, a HungryAnt must spend some turns chewing before being able to eat again. 
    While the HungryAnt is chewing, it is not able to eat (deal damage to) any Bees. 
    ...
    def action(self, gamestate):
        if self.cooldown > 0:
            ...
            if self.place.bees:
                ...
                Ant.reduce_health(target_bee, target_bee.health)
                # altetnate statement
...
class TankAnt(ContainerAnt):
    """TankAnt is a ContainerAnt that protects an ant in its place and also 
    deals 1 damage to all bees in its place each turn. Like any ContainerAnt, 
    a TankAnt allows the contained ant to perform its action each turn.
    """
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower, a subclass of ThrowerAnt, is more costly and waterproof, 
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            ...
            # Override the bee's action method with the slowed action
...
class NinjaAnt(Ant):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        NinjaAnt.blocks_path = False

    def action(self, gamestate):
        # BEGIN Problem EC 3
        "*** YOUR CODE HERE ***"
        its_place = self.place
        for bee in its_place.bees.copy():       # duplicate against failure of iteration
            bee.reduce_health(self.damage)
        # END Problem EC 3
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        # count insects in front of Laser
        while current_place and not current_place.is_hive:
            distance += 1
            if current_place.ant:
                insects_and_distances[current_place.ant] = distance
                if isinstance(current_place.ant, ContainerAnt) and \
                current_place.ant.ant_contained:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        current_damage = self.original_damage -  0.25 * distance \
