...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    filtered_res = [segment for segment in paragraphs if select(segment)]
    return '' if len(filtered_res) <= k else filtered_res[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def selector(string):
        split_str_list = split(lower(remove_punctuation(string)))
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    t_size, s_size = len(typed_words), len(source_words)
    if t_size == 0:
        return 0.0 if s_size else 100.0
    elif s_size == 0:
        return 0.0
    
    cnt = 0
    for i in range( min(t_size, s_size) ):
        cnt += ( typed_words[i] == source_words[i] )
    return 100 * cnt / t_size
    # END PROBLEM 3
...
def furry_fixes(typed, source, limit):
    ...
    def helper(typed, source, limit, idx):
        if limit == -1: 
            return 0
        if idx == min(len(typed), len(source)):
            return abs(len(typed) - len(source))
        if typed[idx] != source[idx]:
            limit -= 1
        return ( typed[idx] != source[idx] ) + helper(typed, source, limit, idx + 1)
...
def minimum_mewtations(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0: # Feel free to remove or add additional cases
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 1 + min(add, min(remove, substitute))
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    for t, s in zip(typed, source):
        if t == s:
            ...
            break
    ratio = correct_count / len(source)
    archive = {'id': user_id, 'progress': ratio}
    upload(archive)
...
def fastest_words(words_and_times):
    ...
    for word_idx in word_indices:
        fast_id, fast_time = 0, float('inf')
        for player_idx in player_indices:
            if fast_time > get_time(times, player_idx, word_idx):
            # if fast_time > times[player_idx][word_idx]:
                fast_time = get_time(times, player_idx, word_idx)
                fast_id = player_idx
        res[fast_id].append( words[word_idx] )
    return res
    # END PROBLEM 10
