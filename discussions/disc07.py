...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        self.keys = {c: Button(c, (lambda x: None)) for c in LOWERCASE_LETTERS}  # Try a dictionary comprehension!
    ...
    def type(self, word):
        ...
        for c in word:
            self.keys[c].press()
            # Alternate implementation
            # if isinstance(self.keys.get(c, 0), Button):
            #     self.keys[c].press()
            # else:
            #     self.keys[c] = Button(c, (lambda x: None)).press()
            
            if Button.caps_lock.pressed % 2 == 1:
                self.typed.append(c.upper())
            else:
                self.typed.append(c)
