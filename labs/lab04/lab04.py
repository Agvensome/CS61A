...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_a_lat, city_b_lat = get_lat(city_a), get_lat(city_b)
    city_a_lon, city_b_lon = get_lon(city_a), get_lon(city_b)
    
    diff_a = sqrt((city_a_lat - lat)**2 + (city_a_lon - lon)**2)
    diff_b = sqrt((lat - city_b_lat)**2 + (lon - city_b_lon)**2)
    return get_name(city_b) if diff_a > diff_b else get_name(city_a)
