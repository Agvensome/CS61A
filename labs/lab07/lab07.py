class Account:
    ...
    def time_to_retire(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        time, amount_after_time = 0, self.balance
        while amount_after_time < amount:
            time += 1
            amount_after_time *= (1 + self.interest)
        return time
...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        if self.free_withdrawals > 0:
            ...
            # one instruction, takes in two arguments
            return Account.withdraw(self, amount)
        else:
            # alternate instruction, takes in an argument
...
def without(s, i):
    ...
    dummy = Link(-1)
    current_new = dummy
    while current_s != Link.empty:
        if idx != i:
            current_new.rest = Link(current_s.first)
        else:
            current_s = current_s.rest
            current_new.rest = Link(current_s.first)
        idx += 1
        current_s = current_s.rest
        current_new = current_new.rest
    return dummy.rest
