...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    if is_mobile(m) and is_arm(left(m)) and is_arm(right(m)):
        return length(left(m)) * total_mass(end(left(m))) == \
            length(right(m)) * total_mass(end(right(m))) \
...
def berry_finder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if is_tree(t) and label(t) == 'berry':
        return True
...
def max_path_sum(t):
    ...
    for branch in branches(t):
        max_branch_val = max(max_branch_val, max_path_sum(branch))
    return max_branch_val + label(t)
